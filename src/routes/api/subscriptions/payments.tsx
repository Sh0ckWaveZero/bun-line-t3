...
export async function GET(request: Request) {
  try {
    ...
    const billingMonth =
      searchParams.get("billingMonth") ?? getCurrentMonthLabel();
    ...
    if (!subscriptionId) {
      return Response.json(
        { error: "กรุณาระบุ subscriptionId หรือ memberId" },
        { status: 400 },
      );
    }
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json(
      { error: "ไม่สามารถอัปเดตการจ่ายเงินได้" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!subscriptionId) {
      return Response.json(
        { error: "กรุณาระบุ subscriptionId" },
        { status: 400 },
      );
    }
