...
export const Route = createFileRoute("/api/health/enhanced")({
  server: {
    handlers: {
...
async function GET(request: Request) {
  ...
  try {
    ...
    if (process.memoryUsage) {
      ...
      if (healthCheck.metrics.memoryUsage.percentage > 90) {
        ...
        recommendations.push(
          "Consider restarting the application or scaling up",
        );
    ...
    try {
      const missingVars = REQUIRED_ENV_KEYS.filter(
        (key) => !process.env[key],
      ).length;
    ...
    const response = Response.json(healthCheck, {
      status: httpStatus,
      headers: {
