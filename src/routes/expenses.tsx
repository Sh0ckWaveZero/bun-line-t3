    ...
    <div id="expenses-page" className="bg-background min-h-screen w-full">
      <div className="container mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => catFlow.setShowManagerModal(true)}
                className="gap-2"
              >
                <Tag size={14} />
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
          <SummaryCard
            ...
            icon={<TrendingUp className="text-card-green h-4 w-4 sm:h-5 sm:w-5" />}
          ...
          <SummaryCard
            ...
            icon={<TrendingDown className="text-card-red h-4 w-4 sm:h-5 sm:w-5" />}
            iconBg="bg-card-red"
            hideAmounts={hideAmounts}
          />
          <SummaryCard
            ...
            icon={
              <Wallet
                className={`h-4 w-4 sm:h-5 sm:w-5 ${
                  summary && summary.balance >= 0 ? "text-card-blue" : "text-destructive"
                }`}
              />
            }
            iconBg={summary && summary.balance >= 0 ? "bg-card-blue" : "bg-destructive/10"}
            hideAmounts={hideAmounts}
          />
        </div>
        ...
        <div className="mb-4 sm:mb-6">
          <button
            ...
            className="text-muted-foreground hover:text-foreground flex w-full items-center justify-between rounded-lg px-1 py-2 text-xs font-medium transition-colors"
          >
            <span>กราฟสรุป</span>
            <ChevronDown
              size={14}
            ...
            {TRANSACTION_TABS.map((tab) => (
              <TabsTrigger
                ...
                className={`flex-1 text-xs font-medium sm:text-sm ${
                  tab === "EXPENSE" ? "text-card-red" : tab === "INCOME" ? "text-card-green" : ""
                }`}
              >
      ...
      <div className="fixed bottom-6 left-1/2 z-40 -translate-x-1/2 sm:bottom-8">
        <Button
          ...
          className="h-12 rounded-full bg-primary px-6 text-primary-foreground shadow-xl transition-all hover:scale-105 hover:bg-primary/90 sm:h-14"
        >
      ...
      {/* ─── Modals ─── */}
      <AddTransactionModal
        key={`${txModal.showModal ? "open" : "closed"}-${txModal.editingTx?.id ?? "new"}`}
      ...
      <AddCategoryModal
        key={`${catFlow.showCategoryModal ? "open" : "closed"}-${catFlow.editingCategory?.id ?? "new"}`}
