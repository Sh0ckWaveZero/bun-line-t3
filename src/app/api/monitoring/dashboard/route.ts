...
async function secureMonitoringHandler(
  ...
  try {
    ...
    const healthResponse = await fetch(
      `${process.env.NEXTAUTH_URL || "http://localhost:3000"}/api/health/enhanced`,
      {
        headers: {
...
async function getSystemAlerts(healthAlerts: string[]) {
  const alerts = healthAlerts.map((alert, index) => ({
    id: `alert-${index}`,
    level:
      alert.includes("critical") || alert.includes("failed")
        ? ("critical" as const)
        : alert.includes("high") || alert.includes("elevated")
          ? ("warning" as const)
          : ("info" as const),
    message: alert,
    timestamp: new Date().toISOString(),
