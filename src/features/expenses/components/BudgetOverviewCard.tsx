  ...
  return (
    <TooltipProvider>
      <Card
        ...
        className="border-border/70 bg-card/85 dark:bg-card/70"
      >
        <CardHeader id="budget-header" className="pb-2">
          <div
            id="budget-header-inner"
        ...
        <CardContent id="budget-content" className="space-y-3">
          <div
            id="budget-status-grid"
            className="grid grid-cols-3 gap-2 text-sm"
          >
            <div
              id="budget-status-safe"
              className="flex items-center gap-1.5 rounded-md bg-green-500/10 px-2 py-1 text-green-600 dark:text-green-400"
            >
            ...
            <div
              id="budget-status-near"
              className="flex items-center gap-1.5 rounded-md bg-yellow-500/10 px-2 py-1 text-yellow-600 dark:text-yellow-400"
            >
            ...
            <div
              id="budget-status-over"
              className="flex items-center gap-1.5 rounded-md bg-red-500/10 px-2 py-1 text-red-600 dark:text-red-400"
            >
          ...
          {budgets.length === 0 ? (
            ...
            >
              <p id="budget-empty-msg" className="mb-3 text-sm">
                ยังไม่มีงบประมาณ
              </p>
              {onManageBudgets && (
                <Button
...
function BudgetItem({ budgetUsage, hideAmounts }: BudgetItemProps) {
  ...
  return (
    <div
      ...
      className="border-border/60 bg-background/50 space-y-1.5 rounded-lg border p-2.5"
    >
      <div
        id={`budget-item-header-${budget.id}`}
        className="flex items-center justify-between text-sm"
      >
        <div
          id={`budget-item-left-${budget.id}`}
          className="flex items-center gap-2"
        >
          <span id={`budget-item-icon-${budget.id}`} className="text-base">
            {categoryIcon}
          </span>
          <span id={`budget-item-name-${budget.id}`} className="font-medium">
            {categoryName}
          </span>
        </div>
        <span
          id={`budget-item-pct-${budget.id}`}
      ...
      <Progress
        id={`budget-item-progress-${budget.id}`}
        value={displayPercentage}
        className="h-1.5"
      />

      <div
        id={`budget-item-details-${budget.id}`}
        className="text-muted-foreground flex items-center justify-between text-xs"
      >
        <span id={`budget-item-spent-${budget.id}`}>
          ใช้ {hideAmounts ? "•••" : formatAmount(spent)} /{" "}
          {hideAmounts ? "•••" : formatAmount(budget.amount)}
        </span>
