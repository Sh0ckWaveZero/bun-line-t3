 ...
 */
export async function getLinePermissions(
  lineUserId: string,
): Promise<LinePermissions> {
 ...
 */
export async function canRequestAttendanceReport(
  lineUserId: string,
): Promise<boolean> {
 ...
 */
export async function canReceiveReminders(
  lineUserId: string,
): Promise<boolean> {
  const perms = await getLinePermissions(lineUserId);
