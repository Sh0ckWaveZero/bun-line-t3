...
const DateStringSchema = z.string().refine((date) => !isNaN(Date.parse(date)), 'Invalid date string')
...
export const formatTimeOnly = (date: Date | string | null | undefined): string => {
  ...
  try {
    const validatedDate = typeof date === 'string' 
      ? DateStringSchema.parse(date) 
      : DateSchema.parse(date)
    
    const dateObj = typeof validatedDate === 'string' ? new Date(validatedDate) : validatedDate
...
export const combineOriginalDateWithNewTime = (
  ...
  if (!originalDate || !newTime) {
    console.warn('Missing originalDate or newTime in combineOriginalDateWithNewTime')
    return null
  }
  ...
  try {
    ...
    const originalDateObj = typeof originalDate === 'string' 
    ...
    console.warn('Error combining date and time:', error, { originalDate, newTime })
    return null
  }
}
 ...
 */
export const formatForDateTimeLocal = (date: Date | string | null | undefined): string => {
  if (!date) return ''
 ...
 */
export const parseDateTimeLocal = (datetimeLocal: string | null | undefined): Date | null => {
