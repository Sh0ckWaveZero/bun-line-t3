...
export class D3ChartGenerator {
  ...
  async generateHistoricalChart(
    ...
    // Title
    const chartTitle = title || `${symbol.toUpperCase()} Historical Price Chart (${hours}h)`;
    context.fillText(chartTitle, width / 2, 30);

    // Price change indicator
    context.fillStyle = lineColor;
    context.font = '500 16px Prompt, Arial, sans-serif';
    context.fillText(`${formatPercentageLabel(priceChange)} (${hours}h)`, width / 2, 55);
    ...
    context.fillStyle = CHART_COLORS.TEXT_PRIMARY;
    context.fillText(`Current: ฿${formatCryptoPrice(lastPrice)}`, width - 20, 75);
  ...
  async generateHistoricalComparison(
    ...
    normalizedData.forEach((data, symbolIndex) => {
      ...
      const color = colors[symbolIndex % colors.length] || CHART_COLORS.PRIMARY_BLUE;
    ...
    // X-axis label
    context.fillText(`Time (${hours} hours)`, margin.left + chartWidth / 2, height - 20);
  ...
  private async generateErrorChart(message: string): Promise<Buffer> {
    ...
    context.fillStyle = CHART_COLORS.TEXT_TERTIARY;
    context.font = '400 12px Prompt, Arial, sans-serif';
    context.fillText('Please try again later or check symbol availability', width / 2, height / 2 + 30);
