...
export const validateUrl = (url: string, env: Environment = detectEnvironment()): {
  isValid: boolean
...
export const getSafeRedirectUrl = (
  ...
  console.warn(`🚨 Security: Blocked unsafe redirect attempt to "${url}". Reason: ${validation.error}`)
...
export const sanitizeUrl = (url: string): string => {
  try {
    ...
    const dangerousParams = ['javascript:', 'data:', 'vbscript:', 'onload', 'onerror', 'onmouseover', 'onclick']
    ...
    for (const [key, value] of searchParams.entries()) {
      ...
      const isDangerous = dangerousParams.some(param => 
...
export const validateNextAuthUrl = (url: string): {
  isValid: boolean
