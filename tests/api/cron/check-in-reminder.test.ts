...
describe('Check-in Reminder API', () => {
  ...
  describe('Holiday Detection from MongoDB', () => {
    it('should skip reminder on public holidays', () => {
      ...
      expect(mockHolidayResponse.holidayInfo?.nameEnglish).toBe('Songkran Festival');
    ...
    it('should include different types of holidays', () => {
      ...
      holidayTypes.forEach(type => {
        ...
        expect(['national', 'royal', 'religious', 'special']).toContain(mockHolidayResponse.holidayInfo.type);
      });
    });
  });
  ...
  describe('Database Integration (Mocked)', () => {
    it('should handle holiday data structure correctly', () => {
      ...
      expect(['national', 'royal', 'religious', 'special']).toContain(mockHolidayFromDB.type);
    });
  ...
  describe('Random Selection Logic', () => {
    const testArray = ['message1', 'message2', 'message3', 'message4', 'message5'];
  ...
  describe('API Response Structure', () => {
    it('should have proper success response format', () => {
      const mockSuccessResponse = {
        ...
        messageText: '🌅 สวัสดีตอนเช้า! วันใหม่ที่สดใส เริ่มต้นด้วยการลงชื่อเข้างานกันนะคะ ✨',
