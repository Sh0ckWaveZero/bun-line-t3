...
describe('Enhanced Health Check API Tests', () => {
  ...
  test('should calculate health score correctly', async () => {
    ...
    const mockDegradedHealth = {
      ...
      recommendations: ['Configure LINE_CHANNEL_ACCESS_TOKEN and LINE_CHANNEL_SECRET']
    }
...
describe('API Rate Limiting Integration', () => {
  test('should add rate limit headers to responses', async () => {
    ...
    expect(parseInt(mockHeaders['X-RateLimit-Remaining'])).toBeLessThanOrEqual(5)
    expect(parseInt(mockHeaders['X-RateLimit-Reset'])).toBeGreaterThan(Math.floor(Date.now() / 1000))
  })
  ...
  test('should handle rate limit exceeded scenario', async () => {
    ...
    expect(new Date(mockRateLimitResponse.resetTime).getTime()).toBeGreaterThan(Date.now())
  })
})
