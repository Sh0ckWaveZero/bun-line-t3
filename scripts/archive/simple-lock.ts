...
class SimpleProcessLock {
  ...
  async isRunning(): Promise<boolean> {
    try {
      ...
      try {
        ...
        console.log(
          `⚠️  Process '${this.processName}' is already running (PID: ${lockInfo.pid})`,
        );
        console.log(
          `   Started at: ${new Date(lockInfo.startTime).toLocaleString()}`,
        );
  ...
  async acquireLock(): Promise<void> {
    ...
    console.log(
      `🔒 Process lock acquired for '${this.processName}' (PID: ${this.pid})`,
    );
